...
switch ($tab) {
    case 'all_error':
        ...
        $data['history'] = \DDAProduction\Zeo404\Models\ZeoUrlFail::query()->where(
            'zeo_url_id',
            $_GET['url_id']
        )->orderBy('id', 'DESC')->get();
    ...
    case 'redirect':
        $data['url'] = \DDAProduction\Zeo404\Models\ZeoUrl::find($_GET['url_id'])->url;
        $data['redirects'] = \DDAProduction\Zeo404\Models\ZeoUrlRedirect::query()->where(
            'zeo_url_id',
            $_GET['url_id']
        )->orderBy('id', 'DESC')->get();
        $outTpl = (string)View::make('Zeo::main', $data);
    ...
    case '404info_task':
        ...
        $pages = \DDAProduction\Zeo404\Models\CheckTaskPage::query()
        ...
        if (isset($_GET['orderby']) && $_GET['orderby'] != 'default') {
            $pages = $pages->orderBy($_GET['orderby'], 'DESC');
        } else {
            $pages = $pages->orderBy('updated_at');
        }

        $data['pages'] = $pages->paginate(20)->appends($_GET);
        $outTpl = (string)View::make('Zeo::main', $data);
